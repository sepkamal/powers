...
test_that("At least numeric values work.", {
	...
	expect_identical(power_datatable(num_vec), data.frame(Base = num_vec, Power = num_vec^2))
})
...
test_that("Logicals automatically convert to numeric.", {
	...
	expect_identical(power_datatable(logic_vec), data.frame(Base = logic_vec, Power = logic_vec^2))
})
